...
if __name__ == '__main__':
      ...
      data_dir = args.input_dir                              # 输入的文件夹级别
      orderlist_path = args.input_orderlist                  # 输入的orderlist 是什么？
      readspath = args.input_segmentation                   # 输入的segmentation 是什么？ 应该是每个tile的分割csv ?
      ...
      # 1. Variables initialization and read in data
      # cell_barcode_min = 0
      # middle_edge = 0
      ...
      # Read coordinates
      obs_coords = get_coords(args.Tile_registered)      # 读取观察到的坐标
      exp_coords = get_coords(args.Tile)                # 读取预期的坐标
      grid_order = get_coords(args.Tile, grid=True)    # 读取网格排列布局 
      ...
      grid_df = pd.DataFrame(grid_order, columns=['tile', 'column_index', 'row_index'])
      ...
      shape_column, shape_row = [max_column - min_column + img_c, max_row - min_row + img_r]    # 因为左上角坐标，所以额外加上图像宽度和高度
