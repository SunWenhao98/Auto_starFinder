#!/usr/bin/env bash
set -euo pipefail   # 经典的 bash 严格模式
...
PARAM_TSV="${1:-${BASE}/00_registration/params.tsv}"    # 参数文件路径（必填）
POS_SPEC="${2:-Position012}"                            # 要处理哪些物理视野（如 Position1-100）

# Metadata of the image and work path
Z_NUM=45
XY_SIZE=2048
GENE_CSV="${BASE}/01_data/genes_tri.csv"
IDN="4"
ROUND_DAPI="round001"     # 参考轮次
...
WINDOW_SIZE=300
OVERLAP=0.2
EXPORT_POLYGONS="${EXPORT_POLYGONS:-T}"   # 是否输出多边形坐标
EXPORT_QC_PLOTS="${EXPORT_QC_PLOTS:-T}"   # 是否输出质量控制图像（如细胞轮廓叠加图）
ROTATION="${ROTATION:-90}"        # Leica: 90, Olympus: 0;
READS_FILTER="${READS_FILTER:-5}"
DAPI_SUFFIX="${DAPI_SUFFIX:-ch03.tif}"

# SLURM 队列与资源分配参数 (核心调度设计)
ARRAY_CONCURRENCY=20    # 允许并发的任务数
PARTITION="C64M512G"
QOS="normal"
...
# RESCUE/RETRY (救援/重试)：针对失败的 TILE 进行重算。
RETRY_CPUS=60
RETRY_MEM="480G"
RETRY_TIME="24:00:00"
...
# 参数为 1 时，多组参数会复用同一份预处理结果
# 查当前已运行任务数，超过该数字就暂停投递。0 表示不限制。
SHARE_PREP_ACROSS_PARAMS="${SHARE_PREP_ACROSS_PARAMS:-1}" 
MAX_LIVE_PIPELINES="${MAX_LIVE_PIPELINES:-0}" 
DRY_RUN="${DRY_RUN:-0}"                   # 空跑模式。调试长流程脚本

# 构成了后续 sbatch_retry 函数的指数退避重试逻辑。
SBATCH_RETRIES="${SBATCH_RETRIES:-6}"
SBATCH_RETRY_SLEEP="${SBATCH_RETRY_SLEEP:-2}"

mkdir -p "$LOG_ROOT"
RUN_ID="$(date +%Y%m%d_%H%M%S)_$$"  # 生成唯一的运行 ID（时间戳 + 当前进程 PID $$）。关系：用于后续生成临时模板文件目录，防止多次同时运行脚本时文件相互覆盖。


log(){ echo "$@" >&2; }
need_cmd(){ command -v "$1" >/dev/null 2>&1 || { log "ERROR: missing command: $1"; exit 1; }; } # 先检查所有必需的工具和文件是否存在。
...
# ---------- params TSV ----------
# 读取并清洗 TSV 文件内容存入数组 PARAM_LINES
