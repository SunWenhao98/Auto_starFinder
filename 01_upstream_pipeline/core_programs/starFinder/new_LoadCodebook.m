% function [ geneToSeq, seqToGene ] = new_LoadCodebook( inputPath, remove_index, doReverse, codeMap_mode )
% % new_LoadCodebook
...
%     % reverse sequence of codebook: seqF + seqD 
%     if doReverse
%         f(:,2) = reverse(f(:,2));
%     end

%     % encode barcode: 10 rounds -> 11 bits -> remove index6
%     for i=1:size(f, 1)
...
%     end
%     % display gene_name and barcode, for debug, 两段希望对齐，方便查看，不足位补空格，右对齐
%     disp(f(:,1);f(:, 2));
