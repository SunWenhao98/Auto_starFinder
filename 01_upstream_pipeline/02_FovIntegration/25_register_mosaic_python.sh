...
print_usage() {
    cat <<'USAGE'
Usage: 25_register_mosaic_python.sh --project_root PATH --project_name NAME --reg_dir_suffix DIR --fixed_path PATH --moving_path PATH --output_workdir DIR --output_label NAME [options]
...
for value in PROJECT_ROOT PROJECT_NAME REG_DIR_SUFFIX SCRIPT_DIR CONDA_SH FIXED_PATH MOVING_PATH OUTPUT_WORKDIR OUTPUT_LABEL; do
...
echo "[PARAM] UPSAMPLE_FACTOR=${UPSAMPLE_FACTOR}"
...
for path in "$FIXED_MOSAIC" "$MOVING_MOSAIC" "$CONDA_SH" "$REGISTER_SCRIPT" "$APPLY_SCRIPT"; do
