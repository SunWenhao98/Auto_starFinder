...
def command_args():
    ...
    parser.add_argument('-IXY', '--input_xy', type=int, required=False,help='image width and height in pixels')
    parser.add_argument('-ID', '--input_dir', type=str, required=False, help='the input_dir')
    parser.add_argument('-OD', '--output_dir', type=str, required=False, help='output_cell_center')
    parser.add_argument('-SM', '--seg_method', type=str, required=False, help='clustermap or watershed')
    parser.add_argument('-IF', '--IF_dirname', type=str, required=False)
...
if __name__ == '__main__':
    ...
    # 3. 输出路径：输出cell_centers.csv, remain_reads.csv的integration文件夹路径
    # 4. 分割方法：clustermap or watershed
    # 5. 存放拼接缝合后坐标文件夹名：IF_dirname；组合后访问TileConfiguration.registered.txt等


    img_c, img_r = args.input_xy, args.input_xy
...
#   坐标读取和元信息整合
    ...
    coords_df.to_csv(os.path.join(output_path,'coords.csv'))


# 2. tune coordinates and calculate grid dimensions
    ...
    grid_column = len(np.unique(coords_df_without_blank['column_coord_exp'] // (img_c * 0.9)))
    grid_row = len(np.unique(coords_df_without_blank['row_coord_exp'] // (img_r * 0.9)))
    print("Grid dimensions: ", grid_column, "-x-", grid_row)

    # Save tuned coordinates
    coords_df_tuned.to_csv(os.path.join(output_path,'tuned_coords.csv'))

    ## 3. Stitching reads and cells
    print("============STITCHING============")
    cell_barcode_min = 0
    ...
    cell_center = pd.DataFrame({'cell_barcode':[],'column':[],'row':[],'z':[],'raw_cell_barcode':[],'gridc_gridr_tilenum':[]})

    # Create mapping of tile numbers to grid positions
    tile_to_grid = {}
    for _, row in coords_df_tuned.iterrows():
        if row['tile'] > 0:  # Skip blank tiles
                tile_num = int(row['tile'])         # 1-based
                grid_x = int(row['column_index'])
                grid_y = int(row['row_index'])
    ...
    for tilenum in sorted_tiles:
        ...
        median_col = coords_df_tuned[(coords_df_tuned.column_index == t_grid_c) & (coords_df_tuned.tile != 0)]['column_coord_obs']
        median_row = coords_df_tuned[(coords_df_tuned.row_index == t_grid_r) & (coords_df_tuned.tile != 0)]['row_coord_obs']
        ...
        try:
                order = coords_df_tuned.index[(coords_df_tuned['column_index']==t_grid_c) & (coords_df_tuned['row_index']==t_grid_r)][0]
                print(f"Tile order: {order}")
        except IndexError:
    ...
    #     print(f"Tile order: {tile_order}")
        ...
        # dfpath = input_regpath + f'/Position{tile_order:03d}/seg/{seg_method}*/'
        # print(f"\nProcessing tile {tile_order}:")
        # print(f"Reading from path: {dfpath}")
        
        # if not os.path.exists(os.path.join(dfpath, 'remain_reads_assigned.csv')):
        #         print(f"- Tile {tile_order}: Reads file does not exist. [{t_grid_c},{t_grid_r}]")
        #         continue
                
        # remain_reads_t = pd.read_csv(os.path.join(dfpath,'remain_reads_assigned.csv'),index_col=0)
        # cell_center_t = pd.read_csv(os.path.join(dfpath,'cell_center.csv'),index_col=0)
        # remain_reads_t = remain_reads_t.reset_index()
        ...
        remain_reads_t = pd.read_csv(reads_file, index_col=0)
        cell_center_t = pd.read_csv(center_file, index_col=0)
        remain_reads_t = remain_reads_t.reset_index()
    ...
    #     print(reads_within_cells.head())
    #     print(reads_process_filtered.head())
    #     print(cell_center_t.head())
    #     print(f"Within Cells Cols: '{reads_within_cells.columns.tolist()}'")
    #     print(f"Process Filtered Cols: '{reads_process_filtered.columns.tolist()}'")
        ...
        # 保存当前tile的结果
        print(cell_center.head())
        print(remain_reads.head())
